                    ...
                    try {
                        ...
                        if (CollectionUtils.isNotEmpty(groupList)) {
                            ...
                            XxlJobAdminConfig.getAdminConfig().getXxlJobRegistryDao()
                                    .removeDead(Date.from(LocalDateTime.now().minusSeconds(RegistryConfig.DEAD_TIMEOUT)
                                            .atZone(ZoneId.systemDefault()).toInstant()));

                            // fresh online address (admin/executor)
                            HashMap<String, List<String>> appAddressMap = new HashMap<String, List<String>>();
                            List<XxlJobRegistry> list = XxlJobAdminConfig.getAdminConfig().getXxlJobRegistryDao()
                                    .findAll(Date.from(LocalDateTime.now().minusSeconds(RegistryConfig.DEAD_TIMEOUT)
                                            .atZone(ZoneId.systemDefault()).toInstant()));
                            if (list != null) {
