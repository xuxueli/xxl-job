...
public final class CronExpression implements Serializable, Cloneable {
    ...
    static final int SECOND = 0; // final 类无法被继承，无需被 protected 修饰
    ...
    static final Map<String, Integer> dayMap = new HashMap<>();
    static {
        monthMap.put("JAN", 0);
        monthMap.put("FEB", 1);
        monthMap.put("MAR", 2);
    ...
    int storeExpressionVals(int pos, final String s, int type) throws ParseException {
        int incr = 0;
    ...
    int checkNext(int pos, final String s, int val, int type)
        ...
        if (c == '-') {
            ...
            if (i < length && ((c = s.charAt(i)) == '/')) {
                i++;
                c = s.charAt(i);
                int v2 = Character.digit(c, 10);
                i++;
                if (i >= length) {
    ...
    String getExpressionSetSummary(Collection<Integer> set) {
        ...
        for (Integer val : set) {
            ...
            buf.append(val.intValue()); // 避免冗余的 Integer.toString() 调用
        }
        return buf.toString();
    }
    ...
    int findNextWhiteSpace(int i, String s) {
        // FIXME 这里疑似有 bug，因为 (s.charAt(i) != ' ' || s.charAt(i) != '\t') 恒为 true
