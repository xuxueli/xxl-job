...
public class RemoteHttpJobBean extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        XxlJobInfo jobInfo = DynamicSchedulerUtil.xxlJobInfoDao.load(jobKey.getGroup(),
                jobKey.getName());
        // save log
        ...
        params.put(HandlerParamEnum.LOG_ADDRESS.name(),
                XxlJobLogCallbackServer.getTrigger_log_address());
        ...
        params.put(HandlerParamEnum.JOB_TYPE.name(), String.valueOf(jobInfo.getJobType()));
        ...
        params.put(HandlerParamEnum.JOB_PATH.name(), jobInfo.getJobPath());
        ...
        jobLog.setExecutorHandler(handler);
        jobLog.setExecutorParam(jobInfo.getExecutorParam());
        logger.info(">>>>>>>>>>> xxl-job failoverTrigger response, jobId:{}, callback:{}",
                jobLog.getId(), callback);
    ...
    public RemoteCallBack failoverTrigger(String handler_address,
                                          HashMap<String, String> handler_params,
                                          XxlJobLog jobLog) {
        if (handler_address.split(",").length > 1) {
            ...
            for (String address : handler_address.split(",")) {
                if (StringUtils.isNotBlank(address)) {
                    HashMap<String, String> params = new HashMap<String, String>();
                    params.put(HandlerParamEnum.TIMESTAMP.name(),
                            String.valueOf(System.currentTimeMillis()));
                    params.put(HandlerParamEnum.ACTION.name(), ActionEnum.BEAT.name());
                    RemoteCallBack beatResult = HttpUtil.post(HttpUtil.addressToUrl(address),
                            params);
                    failoverMessage += MessageFormat.format(
                    ...
                    if (RemoteCallBack.SUCCESS.equals(beatResult.getStatus())) {
                        ...
                        RemoteCallBack triggerCallback = HttpUtil
                                .post(HttpUtil.addressToUrl(address), handler_params);
            ...
            RemoteCallBack triggerCallback = HttpUtil.post(HttpUtil.addressToUrl(handler_address),
                    handler_params);
            String failoverMessage = MessageFormat.format(
                    "Trigger running, <br>>>>[address] : {0}, <br>>>>[status] : {1}, <br>>>>[msg] : {2} <br><hr>",
