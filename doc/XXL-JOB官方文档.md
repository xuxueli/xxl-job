...
- 6、登录界面取消默认填写的登录账号密码；
- 7、[ING]cron在线生成工具；
- 8、执行器表属性调整，"顺序" 属性调整为整型，解决执行器数据较多时无法正确排序的问题；
...
- 9、执行器端口复用，http通讯自动添加相对地址 "/xxl-job"：
- 10、流程任务，支持参数传递；
...
- 25、注册中心优化，实时性注册发现：心跳注册间隔10s，refresh失败则首次注册并立即更新注册信息，心跳类似；30s过期销毁；
- 26、提供执行器Docker镜像；
- 27、脚本任务，支持数据参数，新版本仅支持单参数不支持需要兼容；
