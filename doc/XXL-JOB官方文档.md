...
- 9、[迭代中]自定义失败重试时间间隔；
- 10、[迭代中]任务复制功能；点击复制是弹出新建任务弹框，并初始化被复制任务信息；
- 11、[迭代中]新增执行器描述、任务描述属性；
- 12、[迭代中]任务执行一次的时候指定IP；
- 13、[迭代中]移除旧类注解JobHandler，推荐使用基于方法注解 "@XxlJob" 的方式进行任务开发；
- 14、[迭代中]任务日志支持单个清理和状态转移，方便触发子任务；
...
- 22、通讯调整；双向HTTP，回调和其他API自定义AccessToken，Restful，执行器复用容器端口；
