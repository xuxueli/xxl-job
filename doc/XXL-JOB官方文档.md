...
- 2、任务日志查询速度优化，百万级别日志量搜索速度提升1000倍；
- 3、底层LocalCache组件兼容性优化，支持jdk、jdk10编译部署；
- 4、任务回调结果优化，支持展示在Rolling log中，方便问题排查；
- 5、失败重试策略扩展，细分为"触发失败重试"和"执行失败重试"；
- 6、Glue(Shell) 等脚本任务支持失败重试；
- 7、告警邮件中展示失败告警信息；
- 8、任务属性枚举 "任务模式、阻塞策略" 国际化优化；
- 9、任务日志表状态字段类型优化；
- 10、告警邮箱支持SSL配置；
- 11、Window机器下File.separator不兼容问题修复；
- 12、脚本任务异常Log输出优化；
- 13、任务线程停止变量修饰符优化；
- 14、脚本任务Log文件流关闭优化；
